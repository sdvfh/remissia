...
dic_variaveis = pd.read_csv('../dicionarios/{0}'.format(dic_variaveis), header=None)
...
for base_path in glob.iglob('{0}/**/*.TXT'.format(path_base_origem), recursive=True):
    antigo = time.time()
    path_base_destino_temp = os.path.join(dir_base_destino,
    ...
    print('Tempo: {0} s | Salvo em: {1}'.format(round(time.time() - antigo, 2), path_base_destino_temp))
